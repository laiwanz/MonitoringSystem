...
int RedisParse::config(FUNCTION_PUSH_TASK func, FUNCTION_SHOW_TASK func1, FUNCTION_ACK_TASK func2){
...
int RedisParse::parseRedisMessage(const string& sMessage){
    ...
    if (root.isMember("MachineInfo")) {
        ...
        if (root["MachineInfo"].isMember("Version")){
            reportInfo.sVersion  = root["MachineInfo"]["Version"].isString() ? root["MachineInfo"]["Version"].asString() : "";
        }
        ...
        if (root["Tasks"].isMember("TaskType")){    
            ...
            case 1:
                ...
                break;
            case 2:
                taskInfo.TaskName = REDIS_TASK_TYPE_RE_CONFIG;
                break;
            case 3:
                taskInfo.TaskName = REDIS_TASK_TYPE_CHANGE_CHANNEL;
                break;
            case 4:
    ...
    } else if (root.isMember("Respond")) {


        REDIS_RESPOSE_INFO_T respondInfo;
        if(root["Respond"].isMember("Channel")){
            respondInfo.sChannel = root["Respond"]["Channel"].isString() ? root["Respond"]["Channel"].asString() : "";
        }
        if(root["Respond"].isMember("DeviceId")){
            respondInfo.sDeviceId = root["Respond"]["DeviceId"].isString() ? root["Respond"]["DeviceId"].asString() : "";
        }
        if(root["Respond"].isMember("ACK")){
            respondInfo.bRes = root["Respond"]["ACK"].isBool() ? root["Respond"]["ACK"].asBool() : false;
        }
        return this->m_func2(respondInfo);
...
int RedisParse::newReportInfo(string &ReportInfo, const string &sChannel, const string &sIp, const string &sMAC, const string &sModel,
    ...
    root["MachineInfo"]["CpuPercent"] = nCpu;
